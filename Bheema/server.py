...
def build_system_instructions() -> str:
    ...
    # Build instructions dynamically based on script structure
...
**CORE RULES:**
{chr(10).join(f'{i+1}. {rule}' for i, rule in enumerate(sp['core_rules']))}

**CONVERSATION SCRIPT (Follow this Flow):**

"""
    
    # Dynamically add all phases from the script
    for phase_key, phase_content in script.items():
        if isinstance(phase_content, dict) and 'title' in phase_content:
            instructions += f"**{phase_content['title']}**\n"
            # Add all fields from this phase
